...
const ReflessoDArgento = ({ segment }: { segment?: Segment }) => {

  const segmentPath = segment ? `/${segment}` : '';
  const copy = segment ? copyBySegment['reflesso-d-argento']?.[segment] : undefined;
  const baseCanonical = `https://insulfilm.com.br/arquitetonico${segmentPath}/solar/reflesso-d-argento`;
  ...
  return (
    <>
      ...
      <main>
        ...
        <TechSpecsHighlight
          title="Ficha Técnica Reflesso d'Argento"
          subtitle="Acabamento espelhado prata com performance térmica máxima."
          highlights={[
            { value: '80%', label: 'Rejeição IR', sublabel: 'Calor bloqueado' },
            { value: '78%', label: 'Energia Solar', sublabel: 'Rejeitada' },
            { value: '>99%', label: 'Bloqueio UV', sublabel: 'Proteção total' },
            { value: '3 anos', label: 'Garantia', sublabel: 'INSULFILM™' },
          ]}
          techTable={techTable}
          warrantyNote="Quanto menor o grau de transparência, maior a refletividade percebida após a aplicação. Garantia: até 3 anos. Consulte condições com um Centro Autorizado."
        />
        ...
        <PDPFaqSection slug="reflesso-d-argento" />
      </main>
    </>
  );
};

export default ReflessoDArgento;
