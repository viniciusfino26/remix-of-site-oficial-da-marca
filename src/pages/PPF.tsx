...
const protections = [
  'Pedras', 'Granizo', 'Manchas', 'Insetos e Pássaros',
  'Riscos', 'Lascas', 'Arranhões', 'e muito mais',
];

const highlights = [
  'Imperceptível', 'Brilho Intenso', 'Regenerativo', 'Hidrofóbico', 'Anti-Amarelamento',
];

const compositionLayers = [
  {
    title: 'Selante de brilho intenso',
    desc: 'Revestimento que potencializa o brilho da pintura.',
    icon: Sparkles,
  },
  {
    title: 'Camada transparente hidrofóbica',
    desc: 'Revestimento de poliuretano flexível com propriedades anti-contaminante, hidro-repelente e regenerativa para correção de arranhões superficiais leves.',
    icon: Droplets,
  },
  {
    title: 'Uretano Termoplástico (TPU)',
    desc: 'Resistente a rachaduras, impactos leves e perfurações. Formulado para excelente rejeição de calor e raios UV.',
    icon: Shield,
  },
  {
    title: 'Adesivo',
    desc: 'Acrílico anti-amarelamento com elasticidade para manuseio e aderência às superfícies curvas.',
    icon: FlaskConical,
  },
  {
    title: 'Forro protetor descartável',
    desc: 'Forro fosco de poliéster para proteção do adesivo.',
    icon: Layers,
  },
];
        ...
        <motion.div initial="hidden" animate="visible" variants={stagger}>
          <motion.div variants={fadeInUp} className="flex justify-center mb-4">
            <Badge className="bg-accent/10 text-accent border-accent/20 text-xs uppercase tracking-widest px-4 py-1.5">
              ...
              Película de Proteção de Pintura | Paint Protection Film (PPF)
            </Badge>
          </motion.div>
          <motion.h1 variants={fadeInUp} className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-primary-foreground mb-5 leading-[0.95]">
            INSULFILM™ <span className="text-accent">Phantom</span>
          </motion.h1>
          ...
          <motion.div variants={fadeInUp} className="mt-8 mb-6">
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-xl mx-auto">
              {protections.map((p, i) => (
                <Badge key={i} variant="outline" className="border-accent/30 text-accent/90 text-xs px-3 py-1">
                  {p}
                </Badge>
              ))}
            </div>
          ...
          <motion.div variants={fadeInUp} className="flex flex-wrap justify-center gap-3 mt-6">
            {highlights.map((h, i) => (
              <span key={i} className="text-xs md:text-sm font-bold uppercase tracking-wider text-primary-foreground/80">
                {i > 0 && <span className="text-accent mx-1">|</span>}
                {h}
              </span>
            ))}
          </motion.div>
      ...
      <div className="container mx-auto px-4">
        <motion.div className="text-center mb-16" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          <motion.h2 variants={fadeInUp} className="text-3xl md:text-5xl font-extrabold text-foreground mb-2">
            Phantom <span className="text-accent">Transparente Gloss</span>
          </motion.h2>
          <motion.p variants={fadeInUp} className="text-muted-foreground font-light max-w-lg mx-auto">
            Filme PPF de última geração com 5 camadas de proteção
          </motion.p>
      ...
      <div className="container mx-auto px-4">
        <motion.div className="text-center mb-14" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          <motion.h2 variants={fadeInUp} className="text-3xl md:text-4xl font-extrabold text-foreground mb-2">
            <Thermometer className="w-8 h-8 inline-block mr-3 text-accent" />
        ...
        <motion.div className="max-w-3xl mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
          <motion.div variants={fadeInUp} className="text-center mb-8">
            <h2 className="text-3xl md:text-4xl font-extrabold text-primary-foreground mb-2">
              <Shield className="w-8 h-8 inline-block mr-3 text-accent" />
              Garantia do Produto
            </h2>
