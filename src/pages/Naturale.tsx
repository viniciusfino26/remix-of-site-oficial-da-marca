...
const specs = [
  ...
  { icon: Palette, label: 'Estética', value: 'Tonalidade Neutra' },
...
const Naturale = () => {
  ...
  const schemaMarkup = {
    ...
    "description": "Elegância e tecnologia de bombardeamento iônico. Tonalidade sutil que rejeita até 81% do calor infravermelho nas versões escuras.",
    "image": "LINK_DA_IMAGEM_AQUI", "url": "https://www.insulfilm.com.br/naturale",
    "additionalProperty": [
      { "@type": "PropertyValue", "name": "Tecnologia", "value": "Sputtered (Neutra)" },
      { "@type": "PropertyValue", "name": "Rejeição de Infravermelho (IR)", "value": "Até 81% nas versões escuras" }
    ]
  };
  ...
  return (
    <>
      <Helmet>
        <title>INSULFILM™ Naturale | Película Arquitetônica Neutra Sputtered</title>
        <meta name="description" content="Elegância e tecnologia de bombardeamento iônico. Tonalidade sutil que rejeita até 81% do calor infravermelho nas versões escuras." />
        <meta property="og:title" content="INSULFILM™ Naturale | Película Arquitetônica Neutra Sputtered" />
        <meta property="og:description" content="Elegância e tecnologia de bombardeamento iônico. Tonalidade sutil que rejeita até 81% do calor infravermelho nas versões escuras." />
        <meta property="og:type" content="product" />
        <meta property="og:image" content="LINK_DA_IMAGEM_AQUI" />
        <meta property="og:url" content="https://www.insulfilm.com.br/naturale" />
        <script type="application/ld+json">{JSON.stringify(schemaMarkup)}</script>
      </Helmet>
      <main>
        <PageBreadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Arquitetônico', href: '/arquitetonico' }, { label: 'Controle Solar', href: '/arquitetonico/solar' }, { label: 'Naturale' }]} />
        <section ref={heroRef} className="relative min-h-[60vh] flex flex-col items-center justify-center bg-carbon-gradient overflow-hidden">
          ...
          <motion.div className="container mx-auto px-4 pt-32 pb-20 relative z-10 text-center" style={{ y: heroTextY, opacity: heroOpacity }}>
            <motion.div initial="hidden" animate="visible" variants={stagger}>
              ...
              <motion.h1 variants={fadeInUp} className="text-4xl md:text-6xl font-extrabold text-primary-foreground mb-4 leading-[0.95]">INSULFILM™ Naturale</motion.h1>
              <motion.p variants={fadeInUp} className="text-lg md:text-xl text-primary-foreground/60 font-light max-w-2xl mx-auto">Elegância e tecnologia de bombardeamento iônico. Tonalidade sutil que rejeita até 81% do calor infravermelho.</motion.p>
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} variants={stagger}>
                <motion.h2 variants={fadeInLeft} className="text-3xl md:text-4xl font-extrabold text-primary-foreground mb-6">Elegância Natural com Tecnologia Sputtered</motion.h2>
                <motion.p variants={fadeInLeft} className="text-primary-foreground/60 font-light leading-relaxed mb-8">A Naturale utiliza tecnologia de bombardeamento iônico (Sputtered) para criar uma película de tonalidade neutra e natural, que se integra perfeitamente à arquitetura sem alterar a estética do projeto.</motion.p>
                <motion.ul className="space-y-4" variants={stagger}>
                  {['Tecnologia Sputtered de bombardeamento iônico', 'Tonalidade neutra que não altera a arquitetura', 'Rejeição de até 81% de IR nas versões escuras', 'Visual discreto e refletividade baixa a moderada'].map((text, i) => (
        ...
        <section className="py-24 bg-background overflow-hidden">
          <div className="container mx-auto px-4">
            <motion.div className="text-center max-w-2xl mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} variants={stagger}>
              ...
              <motion.p variants={fadeInUp} className="text-muted-foreground text-lg font-light mb-8">Descubra a elegância natural da Naturale para o seu projeto.</motion.p>
              <motion.div variants={scaleIn}>
                <Button asChild className="bg-accent hover:bg-accent/90 text-accent-foreground font-bold text-lg px-10 py-6 rounded-xl shadow-premium-lg hover:shadow-premium transition-all">
                  <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent('Olá! Gostaria de saber mais sobre a película INSULFILM Naturale.')}`} target="_blank" rel="noopener noreferrer"><MessageCircle className="w-5 h-5" />Solicitar Orçamento</a>
                </Button>
              </motion.div>
            </motion.div>
          </div>
